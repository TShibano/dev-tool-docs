...
コミット後にコミットメッセージのミスや小さな修正を追加したい場合，`git commit --amend`を使用することができる．
ただし，コミットIDが変わるため，すでにpushしている場合は注意する．
...
`git rebase -i`を用いて，インタラクティブリベースを行う．
`git rebase -i HEAD~N`でリベースするコミットの範囲を指定する(Nはコミット数)．
その次に，エディタが開き，各コミットに対する操作を決定する．
上から順に古いコミットが表示される
操作内容は以下である(よく使うコマンドを載せている)．
